...
export default function CameraViewScreen({ onPhotoSaved, onOpenGallery }) {
  ...
  const takePicture = async () => {
    try {
      ...
      const shot = await camRef.current.takePictureAsync(); // ถ่ายรูป
      const filename = `photo_${Date.now()}.jpg`;
      const dest = FileSystem.documentDirectory + filename;

      // คัดลอกไฟล์จาก cache ไปเก็บถาวร
      await FileSystem.copyAsync({ from: shot.uri, to: dest });

      await addPhoto(dest);       // เพิ่มลง storage
      setThumb(dest);             // อัปเดตรูปย่อ
      onPhotoSaved?.(dest);       // แจ้ง parent component
